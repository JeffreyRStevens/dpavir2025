...
Note that the output of format is a character string. So it is no longer a numeric, and you can't do math on the output of `format()`.
...
To divide numbers into bins, use `cut()`. This creates a factor with levels for each of the bins. You can set the bins with the `breaks` argument by passing a vector with the minimum value, intermediate break points, and maximum value.

**7. Use cut to bin the `penguins` body mass data into the following bins: 0-3500, 3501-4800, and 4801-6300.**
