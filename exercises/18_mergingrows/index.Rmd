...
How can we filter the `airlines` data set to only show the carriers included in `flights2`?
...
How can we filter the `airlines` data set to _exclude_ the carriers included in `flights2`?
...
How do we filter the `airlines2` data set to _exclude_ the carriers included in `flights2`?
...
Let's say we have some data from 2014 that we want to add to the bottom of `flights2`. How would we do that? Here's the new data:

```{r}
flights14
```

Would we use `bind_rows()` or `bind_cols()` to add `flights14` to the bottom of `flights2`? Type the code that would combine `flights2` and `flights14` to create an object called `flights1314`.
...
Now create a vector called `id` that is a sequence from 1 to the number of rows in `flights1314` and bind it as the first column of `flights1314` (ensuring the new column is named `id`).
