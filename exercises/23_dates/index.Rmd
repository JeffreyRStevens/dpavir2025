...
Working with dates requires the `{lubridate}` package, which is now part of the core tidyverse packages if you use tidyverse version 2.0.0 or above (I recommend it!). Otherwise, you need to load it separately.
...
**Calculate the number of days that have passed since the class started on January 23, 2023.**
...
```{r exercise11-check}
grade_result(
  pass_if(~identical(.result, today() - ymd("2023-01-23")))
)
```
